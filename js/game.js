...
export default class Game {
	constructor(dom, sprites) {
		...
		this.actions = new Actions(this, dom, this.tutorial, this.tiles);
	}
	...
	createTiles() {
		const tiles = [];
		for (let i = 0; i < 4; i++) {
			let tile = this.addTowerStats(i);
			this.dom.towerMenu.appendChild(tile);
			tiles.push(tile);

			const tileImg = new Image();
			tileImg.src = `/images/towers/tiles/${tile.type}.png`;
			tile.appendChild(tileImg);

			const towerName = document.createElement("p");
			towerName.innerText = `${tile.cost}¥`;
			tile.appendChild(towerName);

			this.dom[tile.type] = tile;
		}
		return tiles;
	}
	...
	initPosts() {
		this.start =
			this.grid[Math.ceil(Math.random() * 3) + 1][
		...
		this.goal =
			this.grid[Math.ceil(Math.random() * 3) + 15][
				Math.ceil(Math.random() * 10) + 1
			];
		this.goal.value = 0;
	...
	checkHit(attack) {
		...
		if (this.grid[gridCol] && this.grid[gridCol][gridRow]) {
			...
			cell.attack(attack.damage, attack.type === "slow");
			for (let j = 0; j < this.creeps.length; j++) {
				if (cell === this.creeps[j].currentCell) {
					if (attack.type !== "all") attack.hit = true;
				}
			}
		}
	}
